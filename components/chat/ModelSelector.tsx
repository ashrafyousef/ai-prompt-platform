...
const CAPABILITY_LABELS: Record<UiModelCapability, string> = {
  vision: "Vision",
  fast: "Fast",
  best_quality: "Best quality",
  long_context: "Long context",
  structured_output: "JSON/Structured",
};
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        onClick={() => setOpen((p) => !p)}
        disabled={disabled}
        className="flex items-center gap-1.5 rounded-full bg-transparent py-2 pl-3 pr-2 text-sm font-medium text-zinc-600 transition hover:bg-zinc-200 focus:outline-none disabled:opacity-50 dark:text-zinc-400 dark:hover:bg-zinc-800"
        aria-expanded={open}
        aria-haspopup="listbox"
      >
        <Sparkles className="h-3.5 w-3.5 text-violet-500" />
        <span className="max-w-[130px] truncate">{selected.displayName}</span>
        <span className="hidden rounded-full bg-zinc-200 px-1.5 py-px text-[10px] font-medium text-zinc-500 dark:bg-zinc-700 dark:text-zinc-300 md:inline">
          {capabilityHint || selected.provider}
        </span>
      ...
      {open ? (
        <div
          role="listbox"
          className="absolute bottom-full right-0 z-50 mb-2 w-[22rem] rounded-xl border border-zinc-200 bg-white p-1.5 shadow-xl dark:border-zinc-700 dark:bg-zinc-900"
        >
          ...
          {budgetStatus === "warning" ? (
            <div className="mb-1 rounded-lg bg-amber-100/70 px-2.5 py-2 text-[11px] text-amber-900 dark:bg-amber-900/30 dark:text-amber-200">
              Near monthly budget. Premium models may be temporarily unavailable.
            </div>
          ) : null}
          {budgetStatus === "blocked" ? (
            <div className="mb-1 rounded-lg bg-red-100/70 px-2.5 py-2 text-[11px] text-red-900 dark:bg-red-900/30 dark:text-red-200">
              Monthly budget reached. Model access is temporarily restricted.
            </div>
          ) : null}
          {models.map((model) => {
            ...
            return (
              <button
                key={model.id}
                type="button"
                onClick={() => { onModelChange(model.id); close(); }}
                disabled={isDisabled}
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className={`text-sm font-medium ${isActive ? "text-violet-800 dark:text-violet-200" : "text-zinc-800 dark:text-zinc-200"}`}>
                      {model.displayName}
                    </span>
